...
def _get(path: str, *, query: dict[str, Any] | None = None) -> Any:
    ...
    req = urllib.request.Request(url, headers={
        ...
        "User-Agent": "ATM-ORDER010-MoltJobs-PreSpend/3.0",
    })
    try:
...
def _certs(job: dict[str, Any]) -> list[str]:
    raw = _pick(job, "requiredCertifications", "requiredCertification", "requirements.certifications", "requirements.requiredCertifications", "requiredEvalPacks", "requirements.evalPacks")
    out: list[str] = []
    if isinstance(raw, str):
        out = [raw]
    elif isinstance(raw, list):
        for x in raw:
            s = _txt(_pick(x, "id", "slug", "name", "topic")) if isinstance(x, dict) else _txt(x)
            if s:
                out.append(s)
        ...
        if s:
            out.append(s)
    if _pick(job, "requiresCertification", "certificationRequired", "requiresEval") is True and not out:
...
def _funding(job: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    tx = _txt(_pick(job, "escrow.txHash", "escrowTxHash", "fundingTxHash", "escrowTransactionHash"))
    flag = _pick(job, "escrowFunded", "funded", "isFunded", "escrowProtected")
    proven = status in {"FUNDED", "LOCKED", "ESCROWED", "ACTIVE"} or bool(tx) or flag is True
    return proven, {"escrow_status": status or "UNKNOWN", "escrow_tx_present": bool(tx), "funded_flag": flag if isinstance(flag, bool) else "UNKNOWN"}
...
def _fit(job: dict[str, Any]) -> tuple[bool, str]:
    ...
    if any(x in text for x in ("instagram", "video", "physical", "phone call", "outreach", "referral", "recruit", "affiliate", "in-person")):
        return False, "REJECTED_NON_AUTONOMOUS_OR_PROMOTION"
    if any(x in text for x in ("research", "analysis", "writing", "content", "data", "code", "software", "documentation", "fact check", "api", "web")):
...
def _competition(job: dict[str, Any], jid: str) -> tuple[int | str, str]:
    direct = _pick(job, "bidCount", "bidsCount", "_count.bids", "competition.bidCount")
    if isinstance(direct, int):
        return direct, "JOB_OBJECT"
    try:
        return len(_rows(_get("/jobs/" + urllib.parse.quote(jid, safe="") + "/bids"))), "BIDS_ENDPOINT"
    except RuntimeError as exc:
        if "MOLTJOBS_READ_HTTP_403:" in str(exc):
            return "UNKNOWN_403", "BIDS_ENDPOINT_FORBIDDEN"
        raise
...
def main() -> int:
    ...
    for listed in jobs:
        ...
        competition, competition_evidence = _competition(detail, jid)
        competition_known = isinstance(competition, int)
        deadline_current = deadline is not None and deadline > now
        materially_exceeds = net is not None and net > MIN_NET_FOR_CERT_USDC
        qualifies = all((status in {"OPEN", "ACTIVE", "POSTED"}, deadline_current, funded, eligible, fit, materially_exceeds, cert_exact, bool(unheld), competition_known))
        candidates.append({
            "job_id": jid,
            ...
            "competition_bid_count": competition,
