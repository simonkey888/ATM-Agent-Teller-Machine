...
def _validate_static_event(event: dict[str, Any]) -> tuple[str, str]:
...
def reconcile_local_ledger(ledger: PaymentLedger, *, http: HttpJsonClient | None = None) -> dict[str, Any]:
    ...
    for event in _events_since(ledger):
        try:
            tx_hash, payer = _validate_static_event(event)
            ...
            proof = build_validated_proof(
                source="github_issue_x402_event+base_receipt",
    ...
    buyers = len({proof.payout_id_or_txid.lower() for proof in x402})
    return {
