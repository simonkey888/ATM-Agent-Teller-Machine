        ...
        try:
            ...
            while True:
                ...
                if wait == 0:
                    break
                limit_class = None
                if legacy.time.monotonic() >= deadline:
                    ...
                    try:
                        ...
                        elif legacy._tree_bytes(sandbox_root, ceiling=policy.limits.max_file_bytes * 2) > policy.limits.max_file_bytes * 2:
                            limit_class = "FILE_OUTPUT"
                    except (legacy.psutil.NoSuchProcess, legacy.psutil.AccessDenied):
                        pass
                if limit_class:
            ...
            if int(code.value) != 0:
                raise legacy.SandboxError(f"SANDBOX_CHILD_FAILED:{int(code.value)}")
        finally:
            legacy._cleanup_windows_process(win)
