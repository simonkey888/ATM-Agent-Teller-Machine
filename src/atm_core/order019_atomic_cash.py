...
def _taskmarket_freshness_terminal(exc: Exception) -> str:
    ...
    if "not open" in lowered:
        return "CLOSED"
    return "TASKMARKET_FRESHNESS_UNKNOWN"


def _taskmarket_direct_refetch_reconcile(
    candidate: Any,
    adapters: dict[str, Any],
    *,
    origin: str,
) -> dict[str, Any] | None:
    """Narrow read-only fallthrough for board-only TaskMarket rows.

    A1 ranking remains unchanged for normal discovered candidates. Only a
    DIRECT_SOURCE_REFETCH with the real TaskMarket lane is reconciled here:
    authoritative duplicate truth becomes WATCH/RECONCILE and explicit source
    freshness terminals fall through instead of globally reselecting the same
    dead row. Wallet match is duplicate authority, never ATM-origin authority.
    """
    if origin != "DIRECT_SOURCE_REFETCH" or str(getattr(candidate, "source", "") or "").lower() != "taskmarket":
        return None

    adapter = adapters.get("taskmarket")
    lane = getattr(adapter, "lane", None) if adapter is not None else None
    existing_submission = getattr(lane, "existing_submission", None)
    fetch_authoritative = getattr(adapter, "fetch_authoritative", None) if adapter is not None else None
    verify_freshness = getattr(adapter, "verify_freshness", None) if adapter is not None else None
    if not callable(existing_submission) or not callable(fetch_authoritative) or not callable(verify_freshness):
        # Preserve selector/ranking semantics for light test/fake adapters. The
        # production TaskMarket adapter always exposes all three read-only seams.
        return None

    task_id = str(getattr(candidate, "canonical_opportunity_id", "")).split(":", 1)[-1]
    try:
        snapshot = fetch_authoritative(candidate)
    except Exception as exc:
        return {
            "result": "REJECTED_SOURCE_REFETCH",
            "reason": "AUTHORITATIVE_REFETCH_FAILED",
            "first_terminal_rejection_reason": "AUTHORITATIVE_REFETCH_FAILED",
            "reason_detail": str(exc)[:500],
        }

    try:
        existing = existing_submission(task_id)
    except Exception as exc:
        return {
            "result": "REJECTED_SUBMISSION_RECONCILE",
            "reason": "SUBMISSION_HISTORY_UNKNOWN",
            "first_terminal_rejection_reason": "SUBMISSION_HISTORY_UNKNOWN",
            "reason_detail": str(exc)[:500],
        }

    if isinstance(existing, dict):
        return {
            "result": "WATCH_RECONCILE_EXISTING_SUBMISSION",
            "reason": "ALREADY_SUBMITTED",
            "first_terminal_rejection_reason": "ALREADY_SUBMITTED",
            ...
            "atm_origin_attribution": "NOT_PROVEN_BY_WALLET_MATCH",
            "payment_reconcile_state": "WATCH_ACCEPTANCE_AWARD_SETTLEMENT_BASE_RECEIPT",
            "duplicate_effect_allowed": False,
        }

    try:
        verify_freshness(candidate, snapshot)
    except Exception as exc:
        reason = _taskmarket_freshness_terminal(exc)
        return {
            "result": "REJECTED_SOURCE_FRESHNESS",
            "reason": reason,
...
def choose_atomic_swarm_candidate(
    ...
    *,
    ...
    authoritatively re-fetched. A board-only TaskMarket row may be skipped only
    for authoritative duplicate/freshness truth so it cannot globally early-stop
    allocation. Canon and the effect boundary retain all economic authority.
    """
    del hard_cap
    ...
    for canonical_id in eligible_order:
        ...
        taskmarket_reconcile = _taskmarket_direct_refetch_reconcile(candidate, adapters, origin=origin)
        if taskmarket_reconcile is not None:
            ledger.append(
                {
                    "canonical_id": canonical_id,
                    "origin": origin,
                    "external_state_hash": candidate.external_state_hash,
                    **taskmarket_reconcile,
                }
            )
            continue
