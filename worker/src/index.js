...
async function liveStatus(env){
  ...
  try{
    ...
    const merged={...(obs||fallback)};if(radar){merged.radar=radar.radar;merged.platform_health=radar.platform_health;merged.rail_capabilities=radar.rail_capabilities}merged.money_board={...sanitizeBoard(merged.money_board||{}),...(await liveMoney())};return sanitize(merged);
  }catch{fallback.money_board=await liveMoney();return fallback}
}
