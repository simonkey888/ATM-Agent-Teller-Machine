...
async function liveStatus(env){
  const fallback={authority:"UNOBSERVED",runtime_sha:env.ATM_GIT_SHA||"UNKNOWN",state:"UNKNOWN",monthly_target_usd:String(TARGET),zero_spend:true,radar:{opportunities:[]},platform_health:[]};
...
async function page(env){
  const s=await liveStatus(env);const realized=s.realized_withdrawable_usd??s.monthly_realized_withdrawable_usd;const runtime=s.runtime_sha??s.source_sha;const heartbeat=s.cloud_heartbeat_at??s.updated_at;const state=s.state??s.controller_state;const opps=Array.isArray(s.radar?.opportunities)?s.radar.opportunities:[];
  const rows=[["Authority",s.authority??s.host_class],["State",state],["Phase",s.phase],["Heartbeat",heartbeat],["Runtime SHA",runtime],["Cycle",s.cycle],["Target USD",s.monthly_target_usd],["Withdrawable USD",realized],["Gap USD",s.monthly_gap_usd],["Attack now",s.radar?.attack_now_count??0],["Radar opportunities",s.radar?.opportunity_count??opps.length],["Human gates",s.human_gate_count],["Zero spend",s.zero_spend],["Build",env.ATM_GIT_SHA||"UNKNOWN"]];
...
async function verifyCoinPay(raw,signature,secret){
...
async function coinpayWebhook(request,env){
  ...
  const raw=await request.text();if(!(await verifyCoinPay(raw,request.headers.get("x-coinpay-signature"),env.COINPAY_WEBHOOK_SECRET)))return json({ok:false,error:"invalid_signature"},401);
  let event;try{event=JSON.parse(raw)}catch{return json({ok:false,error:"invalid_json"},400)}const allowed=new Set(["payment.confirmed","payment.forwarded","escrow.settled"]);if(!allowed.has(String(event.type||"")))return json({ok:true,ignored:true},202);
