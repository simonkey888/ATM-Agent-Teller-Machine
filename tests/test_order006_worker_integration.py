...
class Order006WorkerIntegrationTests(unittest.TestCase):
    ...
    def test_readiness_registration_is_not_activation(self):
        zungun = self.integrations.get("zungun")
        across = self.integrations.get("across-edge")
    ...
    def test_boqa_and_senex_are_placeholders_without_final_source_pin(self):
        for worker_id in ("boqa", "senex-prophet"):
            ...
            self.assertIsNone(record.source_pin)
            self.assertIsNone(record.source_pin_ancestor)
            self.assertFalse(self.manifests.get(worker_id).enabled)

    def test_structured_router_positive_negative_matrix(self):
        zungun_job = self._spec(
            name="network_reliability",
            capabilities=["git", "filesystem", "shell", "node", "evidence", "zungun.network_resilience", "zungun.reconciliation"],
        )
        across_job = self._spec(
            name="web3_readonly",
            capabilities=["web3_readonly", "unsigned_transaction_validation", "evidence"],
        )
        ...
        self.assertIsNone(self.integrations.route_registered(browser_job, self.manifests))
        self.assertIsNone(self.integrations.route_registered(research_job, self.manifests))
        self.assertIsNone(self.integrations.route_registered(unknown_job, self.manifests))

        self.assertIsNone(self.integrations.route_active(zungun_job, self.manifests))
        self.assertIsNone(self.integrations.route_active(across_job, self.manifests))

    def test_free_text_coincidence_cannot_route(self):
        job = WorkerJobSpec(
            job_id="order006-free-text",
            canonical_opportunity_id="fixture:order006:free-text",
            external_source="network resilience reconciliation across web3 zungun",
            external_url="https://github.com/simonkey888/ATM-Agent-Teller-Machine/issues/31",
            task_type="browser_qa",
            frozen_acceptance_criteria=["browser only"],
            repository_or_input="network resilience retry reconciliation web3 across edge",
            max_spend_usd=0,
            required_capabilities=["browser"],
            structured_requirements=[],
            expected_deliverable="browser evidence",
    ...
    def test_nonzero_spend_and_signing_or_broadcast_reject(self):
        ...
        with self.assertRaisesRegex(ValueError, "signing/broadcast/financial"):
            self.integrations.route_registered(broadcast, self.manifests)
